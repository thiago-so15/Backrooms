export class HUD {
  ...
  update(state) {
    ...
    let vignette = 0;
    if (sanity < 40) vignette = Math.max(vignette, (40 - sanity) / 40);
    if (entityDistance < 9.5) {
      vignette = Math.max(vignette, (9.5 - entityDistance) / 9.5 * 0.8);
    }
  ...
  _setBar(el, value) {
    ...
    el.style.width = `${pct}%`;
    if (pct > 50) {
      el.style.background = '#c9c17a';
    } else if (pct > 25) {
      el.style.background = '#a89040';
    } else {
      el.style.background = '#b23a3a';
    }
  }
