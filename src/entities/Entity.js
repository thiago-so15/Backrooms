...
const ENTITY_SPEEDS = [0, 1.9, 2.05, 2.2];
const CHASE_MULT = 1.1;
const DETECT_RADIUS = 7.5;
const CATCH_RADIUS = 0.85;
const CHASE_PROB = 0.65;
...
export class Entity {
  ...
  _getSpeed(distToPlayer) {
    let speed = ENTITY_SPEEDS[this.level] || 2.3;
  ...
  _pickNextCell(px, pz, dist) {
    ...
    if (dist < DETECT_RADIUS && Math.random() < CHASE_PROB) {
...
export { DETECT_RADIUS, CATCH_RADIUS };
